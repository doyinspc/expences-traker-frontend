// src/components/SignInForm.tsx

import React, { useState, useCallback, useEffect } from "react";
import { Link, useNavigate } from "react-router-dom";
import { useDispatch, useSelector } from "react-redux";
import Swal from 'sweetalert2';

// Icons
import { ChevronLeftIcon, EyeCloseIcon, EyeIcon } from "../../icons";
import Label from "../form/Label";
import Input from "../form/input/InputField";
import Checkbox from "../form/input/Checkbox";
import Button from "../ui/button/Button";

// Auth imports
import { selectAuth, USER_TYPES } from "../../reducers/auth";
import { adminLogin, tenantLogin, googleLogin, checkAuth } from "../../actions/auth";
import { authActions } from "../../actions/auth";

// Google OAuth configuration
declare const google: any;

// X (Twitter) OAuth configuration
declare const window: any;

// Types
interface LoginResponse {
    success: boolean;
    data?: {
        user: any;
        token: string;
        jwt?: string;
    };
    message?: string;
}

interface FormData {
    email: string;
    password: string;
}

interface Errors {
    email?: string;
    password?: string;
}

export default function SignInForm(): React.ReactElement {
    const navigate = useNavigate();
    const dispatch = useDispatch();
    
    // Redux state using selectAuth
    const authState = useSelector(selectAuth) || {};
    const { isAuthenticated = false, loading = false, error = null, userType = 'guest' } = authState;
    
    const [showPassword, setShowPassword] = useState<boolean>(false);
    const [isChecked, setIsChecked] = useState<boolean>(false);
    const [loginType, setLoginType] = useState<'tenant' | 'admin'>('tenant');
    const [formData, setFormData] = useState<FormData>({
        email: '',
        password: ''
    });
    const [errors, setErrors] = useState<Errors>({});

    // Get environment variables
    const appName: string = import.meta.env.VITE_APP_NAME || 'Expense Tracker';
    const appLogo: string = '/images/logo/logo.svg';
    const appDescription: string = import.meta.env.VITE_APP_DESCRIPTION || 'Manage your expenses efficiently';
    const defaultTenantDB: string = import.meta.env.VITE_APP_TENANT_DB || 'stresert_expences1';
    const googleClientId: string = import.meta.env.VITE_GOOGLE_CLIENT_ID || '';
    const xClientId: string = import.meta.env.VITE_X_CLIENT_ID || '';
    const xRedirectUri: string = import.meta.env.VITE_X_REDIRECT_URI || (window as any).location.origin + '/auth/x/callback';

    // Set dark mode by default
    useEffect(() => {
        document.documentElement.classList.add('dark');
    }, []);

    // Check auth on mount
    useEffect(() => {
        if (checkAuth) {
            dispatch(checkAuth());
        }
    }, [dispatch]);

    // Redirect if already logged in
    useEffect(() => {
        if (isAuthenticated) {
            if (userType === USER_TYPES?.ADMIN) {
                navigate('/admin/dashboard');
            } else if (userType === USER_TYPES?.TENANT) {
                navigate('/');
            } else {
                navigate('/');
            }
        }
    }, [isAuthenticated, userType, navigate]);

    // Show error from Redux if any
    useEffect(() => {
        if (error) {
            Swal.fire({
                icon: 'error',
                title: 'Login Failed',
                text: error,
                confirmButtonColor: '#3085d6',
            });
            if (authActions?.clearError) {
                dispatch(authActions.clearError());
            }
        }
    }, [error, dispatch]);

    // Handle input change
    const handleInputChange = useCallback((e: React.ChangeEvent<HTMLInputElement>): void => {
        const { name, value } = e.target;
        setFormData(prev => ({ ...prev, [name]: value }));
        if (errors[name as keyof Errors]) {
            setErrors(prev => ({ ...prev, [name]: '' }));
        }
    }, [errors]);

    // Toggle login type
    const toggleLoginType = useCallback((): void => {
        setLoginType(prev => prev === 'tenant' ? 'admin' : 'tenant');
    }, []);

    // Validate form
    const validateForm = useCallback((): boolean => {
        const newErrors: Errors = {};
        
        if (!formData.email) {
            newErrors.email = 'Email is required';
        } else if (!/\S+@\S+\.\S+/.test(formData.email)) {
            newErrors.email = 'Please enter a valid email';
        }
        
        if (!formData.password) {
            newErrors.password = 'Password is required';
        } else if (formData.password.length < 6) {
            newErrors.password = 'Password must be at least 6 characters';
        }
        
        setErrors(newErrors);
        return Object.keys(newErrors).length === 0;
    }, [formData]);

    // Handle login submission
    const handleSubmit = useCallback(async (e: React.FormEvent): Promise<void> => {
        e.preventDefault();
        
        if (!validateForm()) return;
        
        let result: LoginResponse | undefined;
        
        // Create FormData object
        const loginFormData = new FormData();
        loginFormData.append('table', 'login');
        
        if (loginType === 'admin') {
            // Admin login
            loginFormData.append('cat', 'login_admin');
            loginFormData.append('username', formData.email);
            loginFormData.append('password', formData.password);
            loginFormData.append('remember', isChecked ? '1' : '0');
            
            result = await dispatch(adminLogin(loginFormData)) as unknown as LoginResponse;
        } else {
            // Tenant login - use tenant_db from env
            loginFormData.append('cat', 'login_tenant');
            loginFormData.append('username', formData.email);
            loginFormData.append('password', formData.password);
            loginFormData.append('tenant_db', defaultTenantDB);
            loginFormData.append('remember', isChecked ? '1' : '0');
            
            result = await dispatch(tenantLogin(loginFormData)) as unknown as LoginResponse;
        }
        
        if (result?.success) {
            console.log('Login successful');
        }
    }, [formData, isChecked, loginType, validateForm, dispatch, defaultTenantDB]);

    // Handle Google login callback
    const handleGoogleCallback = useCallback(async (response: any): Promise<void> => {
        const loginFormData = new FormData();
        loginFormData.append('table', 'login');
        loginFormData.append('cat', 'login_google');
        loginFormData.append('credential', response.credential);
        loginFormData.append('tenant_db', defaultTenantDB);
        
        const result = await dispatch(googleLogin(loginFormData)) as unknown as LoginResponse;
        
        if (result?.success) {
            console.log('Google login successful');
        }
    }, [dispatch, defaultTenantDB]);

    // Handle X (Twitter) login
    const handleXLogin = useCallback((): void => {
        if (!xClientId) {
            Swal.fire({
                icon: 'error',
                title: 'Configuration Error',
                text: 'X (Twitter) login is not configured. Please contact administrator.',
            });
            return;
        }

        sessionStorage.setItem('x_login_data', JSON.stringify({
            tenant_db: defaultTenantDB
        }));

        const state = Math.random().toString(36).substring(7);
        sessionStorage.setItem('x_oauth_state', state);
        
        const authUrl = `https://twitter.com/i/oauth2/authorize?response_type=code&client_id=${xClientId}&redirect_uri=${encodeURIComponent(xRedirectUri)}&scope=tweet.read%20users.read%20offline.access&state=${state}&code_challenge_method=plain&code_challenge=challenge`;
        
        window.location.href = authUrl;
    }, [xClientId, xRedirectUri, defaultTenantDB]);

    // Initialize Google OAuth
    useEffect(() => {
        if (googleClientId && typeof google !== 'undefined' && google.accounts) {
            google.accounts.id.initialize({
                client_id: googleClientId,
                callback: handleGoogleCallback,
            });
            google.accounts.id.renderButton(
                document.getElementById('google-signin-button'),
                { theme: 'outline', size: 'large', width: '100%' }
            );
        }
    }, [googleClientId, handleGoogleCallback]);

    return (
        <div className="flex flex-col flex-1">
            <div className="flex flex-col justify-center flex-1 w-full max-w-md mx-auto px-4 py-6">
                <div>
                    {/* Compact Logo and App Name - Moved to Top */}
                    <div className="flex items-center justify-center gap-3 mb-6">
                        {appLogo && (
                            <img 
                                src={appLogo} 
                                alt={appName} 
                                className="w-10 h-10"
                            />
                        )}
                        <div>
                            <h1 className="text-xl font-bold text-gray-800 dark:text-white/90">
                                {appName}
                            </h1>
                            <p className="text-xs text-gray-500 dark:text-gray-400">
                                {appDescription}
                            </p>
                        </div>
                    </div>

                    {/* Back Button */}
                    <Link
                        to="/"
                        className="inline-flex items-center mb-4 text-xs text-gray-500 transition-colors hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300"
                    >
                        <ChevronLeftIcon className="size-4" />
                        Back
                    </Link>

                    <div className="mb-4">
                        <div className="flex items-center justify-between">
                            <h2 className="font-semibold text-gray-800 text-title-sm dark:text-white/90">
                                Sign In
                            </h2>
                            <button
                                onClick={toggleLoginType}
                                className="text-xs text-brand-500 hover:text-brand-600 dark:text-brand-400"
                            >
                                Switch to {loginType === 'tenant' ? 'Admin' : 'Tenant'}
                            </button>
                        </div>
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                            {loginType === 'admin' 
                                ? 'Admin credentials' 
                                : `${defaultTenantDB}`}
                        </p>
                    </div>
                    
                    <div>
                        {/* OAuth Buttons - More Compact */}
                        <div className="grid grid-cols-2 gap-2">
                            {googleClientId && (
                                <div id="google-signin-button" className="w-full">
                                    <button 
                                        onClick={() => {
                                            if (typeof google !== 'undefined' && google.accounts) {
                                                google.accounts.id.prompt();
                                            }
                                        }}
                                        className="inline-flex items-center justify-center w-full gap-2 py-2 text-xs font-normal text-gray-700 transition-colors bg-gray-100 rounded-lg px-3 hover:bg-gray-200 hover:text-gray-800 dark:bg-white/5 dark:text-white/90 dark:hover:bg-white/10"
                                    >
                                        <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M18.7511 10.1944C18.7511 9.47495 18.6915 8.94995 18.5626 8.40552H10.1797V11.6527H15.1003C15.0011 12.4597 14.4654 13.675 13.2749 14.4916L13.2582 14.6003L15.9087 16.6126L16.0924 16.6305C17.7788 15.1041 18.7511 12.8583 18.7511 10.1944Z" fill="#4285F4"/>
                                            <path d="M10.1788 18.75C12.5895 18.75 14.6133 17.9722 16.0915 16.6305L13.274 14.4916C12.5201 15.0068 11.5081 15.3666 10.1788 15.3666C7.81773 15.3666 5.81379 13.8402 5.09944 11.7305L4.99473 11.7392L2.23868 13.8295L2.20264 13.9277C3.67087 16.786 6.68674 18.75 10.1788 18.75Z" fill="#34A853"/>
                                            <path d="M5.10014 11.7305C4.91165 11.186 4.80257 10.6027 4.80257 9.99992C4.80257 9.3971 4.91165 8.81379 5.09022 8.26935L5.08523 8.1534L2.29464 6.02954L2.20333 6.0721C1.5982 7.25823 1.25098 8.5902 1.25098 9.99992C1.25098 11.4096 1.5982 12.7415 2.20333 13.9277L5.10014 11.7305Z" fill="#FBBC05"/>
                                            <path d="M10.1789 4.63331C11.8554 4.63331 12.9864 5.34303 13.6312 5.93612L16.1511 3.525C14.6035 2.11528 12.5895 1.25 10.1789 1.25C6.68676 1.25 3.67088 3.21387 2.20264 6.07218L5.08953 8.26943C5.81381 6.15972 7.81776 4.63331 10.1789 4.63331Z" fill="#EB4335"/>
                                        </svg>
                                        Google
                                    </button>
                                </div>
                            )}

                            {xClientId && (
                                <button 
                                    onClick={handleXLogin}
                                    className="inline-flex items-center justify-center gap-2 py-2 text-xs font-normal text-gray-700 transition-colors bg-gray-100 rounded-lg px-3 hover:bg-gray-200 hover:text-gray-800 dark:bg-white/5 dark:text-white/90 dark:hover:bg-white/10"
                                >
                                    <svg width="16" className="fill-current" height="16" viewBox="0 0 21 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M15.6705 1.875H18.4272L12.4047 8.75833L19.4897 18.125H13.9422L9.59717 12.4442L4.62554 18.125H1.86721L8.30887 10.7625L1.51221 1.875H7.20054L11.128 7.0675L15.6705 1.875ZM14.703 16.475H16.2305L6.37054 3.43833H4.73137L14.703 16.475Z" />
                                    </svg>
                                    X
                                </button>
                            )}
                        </div>

                        <div className="relative py-2">
                            <div className="absolute inset-0 flex items-center">
                                <div className="w-full border-t border-gray-200 dark:border-gray-800"></div>
                            </div>
                            <div className="relative flex justify-center text-xs">
                                <span className="px-3 text-gray-400 bg-white dark:bg-gray-900">
                                    or
                                </span>
                            </div>
                        </div>

                        <form onSubmit={handleSubmit}>
                            <div className="space-y-4">
                                <div>
                                    <Label className="text-xs">Email</Label>
                                    <Input
                                        name="email"
                                        type="email"
                                        placeholder="info@gmail.com"
                                        value={formData.email}
                                        onChange={handleInputChange}
                                        className={`text-sm ${errors.email ? 'border-red-500' : ''}`}
                                    />
                                    {errors.email && (
                                        <p className="mt-0.5 text-xs text-red-500">{errors.email}</p>
                                    )}
                                </div>

                                <div>
                                    <Label className="text-xs">Password</Label>
                                    <div className="relative">
                                        <Input
                                            name="password"
                                            type={showPassword ? "text" : "password"}
                                            placeholder="Enter your password"
                                            value={formData.password}
                                            onChange={handleInputChange}
                                            className={`text-sm ${errors.password ? 'border-red-500' : ''}`}
                                        />
                                        <span
                                            onClick={() => setShowPassword(!showPassword)}
                                            className="absolute z-30 -translate-y-1/2 cursor-pointer right-3 top-1/2"
                                        >
                                            {showPassword ? (
                                                <EyeIcon className="fill-gray-500 dark:fill-gray-400 size-4" />
                                            ) : (
                                                <EyeCloseIcon className="fill-gray-500 dark:fill-gray-400 size-4" />
                                            )}
                                        </span>
                                    </div>
                                    {errors.password && (
                                        <p className="mt-0.5 text-xs text-red-500">{errors.password}</p>
                                    )}
                                </div>

                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        <Checkbox checked={isChecked} onChange={setIsChecked} />
                                        <span className="text-xs text-gray-700 dark:text-gray-400">
                                            Remember
                                        </span>
                                    </div>
                                    <Link
                                        to="/reset-password"
                                        className="text-xs text-brand-500 hover:text-brand-600 dark:text-brand-400"
                                    >
                                        Forgot?
                                    </Link>
                                </div>

                                <Button 
                                    className="w-full text-sm py-2" 
                                    size="sm"
                                    type="submit"
                                    disabled={loading}
                                >
                                    {loading ? 'Signing in...' : `Sign In`}
                                </Button>
                            </div>
                        </form>

                        <div className="mt-4 text-center">
                            <p className="text-xs text-gray-700 dark:text-gray-400">
                                No account?{" "}
                                <Link
                                    to="/signup"
                                    className="text-brand-500 hover:text-brand-600 dark:text-brand-400"
                                >
                                    Sign Up
                                </Link>
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    );
}