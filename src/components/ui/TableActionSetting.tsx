...
export const TableActionSetting: React.FC<TableActionSettingProps> = ({
    ...
    showActivate = true,
    showDeactivate = true,
    showNext = false, // Next is OFF by default
    is_active,
    size = 'md',
    iconOnly = false,
}) => {
    ...
    // Label text (for icon-only mode, we still show tooltips)
    const labels = {
        edit: 'Edit',
        delete: 'Delete',
        activate: 'Activate',
    ...
    if (showDelete && onDelete) {
        actions.push(
            renderAction('delete', () => onDelete(row))
        );
    }

    // Activate/Deactivate (only show one at a time based on current state)
    if (onActivate) {
        if (isActive && showDeactivate) {
            actions.push(
                renderAction('deactivate', () => onActivate(row))
            );
        } else if (!isActive && showActivate) {
            actions.push(
                renderAction('activate', () => onActivate(row))
            );
        }
    }

    // Next (optional, default OFF)
    if (showNext && onNext) {
        actions.push(
            renderAction('next', () => onNext(row))
        );
    }
    ...
    // For desktop: show all buttons inline
    // For mobile: show dropdown with all actions
    return (
        <div className="inline-flex items-center gap-1">
            ...
            {/* Mobile view - dropdown with all actions */}
...
// Full setting actions (Edit, Delete, Activate/Deactivate)
