// src/pages/Procurement/Userdepartments.tsx
...
// ==================== CONSTANTS ====================
const TABLE_NAME = "userdepartments";
const TABLE_PATH = "userdepartment";
const PAGE_SIZE = 500;
// This is the grp value for departments, adjust as necessary

// ==================== COMPONENT ====================
const Userdepartments: React.FC<UserdepartmentsProps> = ({ 
    UserData = {}, 
    onClose,
    onSave 
}) => {
    ...
    const { 
        ...
        narration: 'get department allocated to current user',
    });
    ...
    const { 
        ...
        queryType: 'gets',
        mainParam: { grp: 2, is_active:1},
        narration: 'get all department current user can permit',
    });
    ...
    // ==================== DepARTMENT OPTIONS ====================
    const processedRowsData = useMemo(() => {

        let departments = departmentData.map((department: any) => ({
            label: department.name ,
            value: department.id 
        }));
        let roles = roleData.map((role: any) => ({
            label: role.name ,
            value: role.id 
        }));
    ...
    useEffect(() => {
        ...
        if (userid && !hasLoaded.current) {
            ...
            loadQuery(initialData);
            loadDepartmentData({ grp: TABLE_PAGE_NUM, userid: staffId });
        }
