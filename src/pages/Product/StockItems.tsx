// src/pages/Procurement/StockItems.tsx
...
const table_data = [
    ...
    {label: 'Stock ID', name:'sku_id', type:'number', showTable: false, editable:true, element:null},
...
const AddItemsModal: React.FC<AddItemsModalProps> = ({
    ...
    const handleRemoveItem = (itemId: number) => {
        if (!window.confirm('Remove this item from the stock?')) return;
        ...
        try {
            ...
            alert('Failed to remove item from stock');
        } finally {
            setAdding(null);
        }
    };
    ...
    // Check if sku_id > 0 (not just exists)
    const itemsWithPO = displayItems.filter((item: any) => item?.sku_id && Number(item.sku_id) > 0 && Number(item.sku_id) === purchaseOrderId);
    const itemsWithoutPO = displayItems.filter((item: any) => !item?.sku_id || Number(item.sku_id) === 0);
                        ...
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                            ...
                            <tbody className="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-800">
                                {displayItems.map((item: any, index: number) => {
                                    ...
                                    // Check if sku_id > 0
                                    const hasPO = item?.sku_id && Number(item.sku_id) > 0;
                                    const isInCurrentPO = item?.sku_id && Number(item.sku_id) === purchaseOrderId;
                                    const hasPON = item?.purchaseorder_id && Number(item.purchaseorder_id) > 0;
                                    ...
                                    return (
                                        <tr key={item?.id || index} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                            ...
                                            <td className="px-3 py-2 whitespace-nowrap text-center">
                                                {isInCurrentPO && !hasPON ? (
                                                    <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300">
                                                        In This STK
                                                    </span>
                                                ) : hasPO  && !hasPON ? (
                                                    <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400">
                                                        In Other STK ({item?.sku_id})
                                                    </span>
                                                ): !hasPO  && hasPON ? (
                                                    <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400">
                                                        In PO ({item?.purchaseorder_id})
                                                    </span>
                                                ) : (
                                            ...
                                            <td className="px-3 py-2 whitespace-nowrap text-center">
                                                {isApproved ? (
                                                    ...
                                                    <span className="text-xs text-gray-400">STK{item?.sku_id}</span>
                                                ) : !hasPO && hasPON ? (
                                                    <span className="text-xs text-gray-400">PO{item?.purchaseorder_id}</span>
                                                ): (
                                                    <button
...
const StockItems: React.FC = () => {
    ...
    const { data, loadQuery, loadUpdate, isLoading } = useReduxApiData({
        ...
        pth: 'requisitionitem',
        queryType: 'gets',
        mainParam: { sku_id: sku_id || 0 },
        narration: 'get stock items'
    });
    ...
    const onDelete = useCallback((row: any): void => {
        ...
        if (window.confirm(`Remove item ${itemName} from this stock?`)) {
            loadUpdate({ id: row?.id, sku_id: null, act: 0 });
        }
    }, [loadUpdate]);
    ...
    const processedRows = useMemo(() => {
        try {
            ...
            return rows.map((row: any) => ({
                ...row,
                // Check if sku_id is greater than 0
                po_assigned: (row?.sku_id && Number(row.sku_id) > 0) ? '✅ Assigned' : '❌ Not Assigned',
            }));
        } catch (error) {
